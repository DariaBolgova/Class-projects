...
#Desired number of nodes input (across the domain)
    ...
    for j in range(0, Ntry):
        ...
        if x > db:  # if the particle is outside the slab
            ...
            if np.random.uniform(0, 1) < PA:  # determination of the reaction type, if it is absorbtion
                prob_surv = wgt/wgt_ave
                prob_abs = abs(1 - prob_surv)
                NA = NA + wgt * prob_abs # count +wht to absorbtions
                wgt = wgt * prob_surv

                if wgt < wgt_low:
                    if np.random.uniform(0, 1) < prob_surv:
                        wgt = wgt_ave
                    else: wgt = 0
                break
